...
// ESPP_CLASS 
#ifndef _INTERACTION_VERLETLISTGMADRESSINTERACTIONTEMPLATE_HPP
#define _INTERACTION_VERLETLISTGMADRESSINTERACTIONTEMPLATE_HPP
...
namespace espressopp {
  namespace interaction {
    ...
    class VerletListGMAdressInteractionTemplate: public Interaction {
    ...
    public:
      VerletListGMAdressInteractionTemplate
    ...
    protected:
      ...
      // Needed for GM-AdResS
      real kbT;
      bool kbT_is_calculated;
      void compute_kbT();
      void getPairEnergies(Particle &, Particle &, real, real);
    };
    ...
    VerletListGMAdressInteractionTemplate < _PotentialAT, _PotentialCG >::
    addForces() {
      ...
      for (PairList::Iterator it(verletList->getAdrPairs()); it.isValid(); ++it) {
         ...
         // GM-AdResS
         real eAT, eCG;
         getPairEnergies(p1, p2, eAT, eCG);
         real gAT = (w12 * exp(-eAT / kbT) / ( w12 * exp(-eAT / kbT) + (1 - w12) * exp(-eCG / kbT)));
    ...
    computeEnergy() {
      ...
      for (PairList::Iterator it(verletList->getAdrPairs()); 
          ...
          real eAT, eCG;
          getPairEnergies(p1, p2, eAT, eCG);
          // GM-AdResS pair "energy" (not conserved)
          e += -kbT * log(w12 * exp(-eAT / kbT) + (1 - w12) * exp(-eCG / kbT));          
      }
    ...
    VerletListGMAdressInteractionTemplate < _PotentialAT, _PotentialCG >::
    computeEnergyAA() {
      std::cout << "Warning! At the moment computeEnergyAA() in VerletListGMAdressInteractionTemplate does not work." << std::endl;
      return 0.0;
    }
    ...
    VerletListGMAdressInteractionTemplate < _PotentialAT, _PotentialCG >::
    computeEnergyCG() {
      std::cout << "Warning! At the moment computeEnergyCG() in VerletListGMAdressInteractionTemplate does not work." << std::endl;
      return 0.0;
    }
    ...
    computeVirialX(std::vector<real> &p_xx_total, int bins) {
      //std::cout << "Warning! At the moment computeVirialX in VerletListGMAdressInteractionTemplate does not work." << std::endl << "Therefore, the corresponding interactions won't be included in calculation." << std::endl;
    ...
    computeVirialTensor(Tensor& w, real z) {
      ...
      std::cout << "Warning! At the moment IK computeVirialTensor in VerletListGMAdress does'n work"<<std::endl;
    ...
    compute_kbT() {
        ...
        if (system.storage->getFixedTuples()){  // AdResS - hence, need to distinguish between CG and AT particles.     
            ...
            for (iterator::CellListIterator cit(realCells); !cit.isDone(); ++cit) {  // Iterate over all (CG) particles.              
                if (cit->lambda() < 1.0) {continue;} //TODO temporary hack, only calculate temperature in atomistic region and assume it's constant across the system, ned to do this because no. of dof not constant in force-Adress
                Particle &vp = *cit;
